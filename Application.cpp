// Протосфера.cpp : Defines the entry point for the application.
//
...
// RenderFX
	...
	// наполнение групп сцены объектами, добавление эффектов
...
void Application::UpdateScene(float dt)
{
	...
	// Изменение скорости перемещения камеры
	p = 1e-2;
	...
	// Изменение количества проходов размытия
	...
	// Задание видовой матрицы камеры
...
void Application::RenderScene()
{
	...
	*Scene.vg_objects_groups["Sphere_Cloth_Group"].effects["RenderFX"].shaders["PS"].constant_buffers["ps_cbPerFrame"] = &ps_cbPerFrame;
	...
	// Отрисовка объектов сцены в текстуру
	Scene.Render();

	// Отрисовка размытой текстуры на плоскости 
