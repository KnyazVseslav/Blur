...
namespace effects
{
	...
	namespace shader_system
	{
		...
		void Shader::AddUAV(ID3D11UnorderedAccessView* UAV, const uint& StartSlot, const uint& NumViews)
		{
			UAVs.push_back(UnorderedAccessView(UAV, setter, StartSlot, NumViews));
		}
		...
		void Texture2D::init(ID3D11ShaderResourceView* SRV, Specification const & specification)
		{
			this->d3dDevice = d3dDevice;
		...
		void Texture2D::assign()
		{
			...
			for (const auto& effect_name : s.FX_names)     // у спецификации берётся i-ое имя эффекта
			  for (const auto& fx_shn : s.shader_names)  // у i-го эффекта берётся i-ое имя шейдера 
			  {
					uint i = 0;
		...
		void Texture2D::set()
		{
			setter->set_texture(StartSlot, NumViews, &SRV); // передать текстуру в шейдер, задаваемый установщиком setter
		}
		...
		UnorderedAccessView::UnorderedAccessView(ID3D11UnorderedAccessView* UAV, ISetter* setter, uint StartSlot, uint NumViews)
		{
			this->UAV = UAV;
			this->StartSlot = StartSlot;
			this->NumViews = NumViews;
		...
		void UnorderedAccessView::set()
		{
			setter->set_UAV(StartSlot, NumViews, &UAV); // передать текстуру в шейдер, задаваемый установщиком setter
		}
		...
		void ComputeShaderSetter::set_UAV(uint StartSlot, uint NumViews, ID3D11UnorderedAccessView* const * UAV)
		{
			d3dDeviceContext->CSSetUnorderedAccessViews(StartSlot, NumViews, UAV, 0);
		}
