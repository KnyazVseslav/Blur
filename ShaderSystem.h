...
template<class T> class ConstantBuffer;
	...
	public:
		...
		virtual void set_UAV(uint StartSlot, uint NumViews, ID3D11UnorderedAccessView* const * UAV) = 0;
	...
	public:
		...
		void set_UAV(uint StartSlot, uint NumViews, ID3D11UnorderedAccessView* const * UAV) {};
...
public:
	...
	void AddUAV(ID3D11UnorderedAccessView* UAV, const uint& StartSlot, const uint& NumViews = 1);
	...
	typedef map_vector<std::string, Texture2D> Textures2DCollection;
	   ...
	   typedef map_vector<std::string, UnorderedAccessView> UAVsCollection;
...
private:
	...
	std::vector<std::string> UAVs_names;
...
public:
	...
	Texture2D() : initialized(false), SRV(0), file_name("") {}
	...
	void init(ID3D11ShaderResourceView* SRV, Specification const & specification);

	void assign(); // вызывается при добавлении объекта в группу


	Texture2D(ID3D11ShaderResourceView* SRV, Specification const & specification);

	void set(); // установка текстуры в один шейдер, связанный с установщиком setter

	void apply(); // пакетный запуск установщиков (текстура передаётся в шейдеры, указанные в спецификации)
...
public:
	...
	UnorderedAccessView(ID3D11UnorderedAccessView* UAV, ISetter* setter, uint StartSlot, uint NumViews);
