...
//float4 r= float4(m[3][0], m[3][1], m[3][2], m[3][3]);
...
//float x_abs = abs(LightDir.x); float y_abs = abs(LightDir.y); float z_abs = abs(LightDir.z);

//return float4(x_abs, y_abs, z_abs, 1);
...
//return float4(L.diffuse.r, L.diffuse.g, L.diffuse.b, 1);
//return float4(L.specular.r, L.specular.g, L.specular.b, 1);
